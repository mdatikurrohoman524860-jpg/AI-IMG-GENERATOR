...
export function ChainEditor({
  ...
  const draftModels = (draftProviderRow?.models ?? []).filter(
    (m) => m.enabled && (capability === 'video' ? m.supportsVideo : m.supportsImages),
  );
  ...
  const skipCount = chain.filter(
    (s) => !matchingProviders.some((p) => p.name === s.provider),
  ).length;
    ...
    <div className="space-y-3">
      ...
      {chain.length === 0 && !variable && (
        <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-2.5 py-2 text-[11px] leading-relaxed text-amber-300">
          No chain yet — add steps below, pick a routing variable, or leave it empty to use the
          tool's default binding (Admin → Tools).
        </p>
      )}
      ...
      {skipCount > 0 && (
        <p className="text-[10px] text-faint">
          {skipCount} step(s) without {capability === 'video' ? 'video' : capability === 'mask' ? 'image/mask' : 'image'} capability — engine skips them at runtime
        </p>
      )}
